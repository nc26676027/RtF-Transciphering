...
func (eval *fvEvaluator) LinearTransform(vec *Ciphertext, linearTransform interface{}) (res []*Ciphertext) {
	vecNTT := &Ciphertext{vec.CopyNew()}
	eval.ringQ.NTT(vec.value[0], vecNTT.value[0])
	eval.ringQ.NTT(vec.value[1], vecNTT.value[1])
	...
	case []*PtDiagMatrixT:
		...
		for i, matrix := range element {
			res[i] = NewCiphertextFV(eval.params, 1)
			eval.MultiplyByDiabMatrix(vecNTT, res[i], matrix, eval.c2QiQDecomp, eval.c2QiPDecomp)
		}
	case *PtDiagMatrixT:
		eval.DecompInternal(vec.value[1], eval.c2QiQDecomp, eval.c2QiPDecomp)

		res = []*Ciphertext{NewCiphertextFV(eval.params, 1)}
...
func (eval *fvEvaluator) MultiplyByDiabMatrix(vecNTT, res *Ciphertext, matrix *PtDiagMatrixT, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
	if matrix.naive {
...
func (eval *fvEvaluator) MultiplyByDiabMatrixNaive(vecNTT, res *Ciphertext, matrix *PtDiagMatrixT, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
	ringQ := eval.ringQ
	ringP := eval.ringP
	...
	for k := range matrix.Vec {
		...
		if k == 0 {
			...
			ringQ.Add(ksResQ0, ct0TimesP, ksResQ0) // phi(d0_Q) += phi(P*c0)
			...
			if cnt == 0 {
				...
				ringQ.MulCoeffsMontgomery(plaintextQ, tmpQ0, res.value[0]) // phi(P*c0 + d0_Q) * plaintext
				ringQ.MulCoeffsMontgomery(plaintextQ, tmpQ1, res.value[1]) // phi(d1_Q) * plaintext
				...
				// keyswitch(c1_Q) = (d0_QP, d1_qp)
				ringQ.MulCoeffsMontgomeryAndAdd(plaintextQ, tmpQ0, res.value[0]) // phi(P*c0 + d0_Q) * plaintext
				ringQ.MulCoeffsMontgomeryAndAdd(plaintextQ, tmpQ1, res.value[1]) // phi(d1_Q) * plaintext
			...
			if cnt%QiOverF == QiOverF-1 {
				ringQ.Reduce(res.value[0], res.value[0])
				ringQ.Reduce(res.value[1], res.value[1])
			}
	...
	if state { // Rotation by zero
		ringQ.MulCoeffsMontgomeryAndAdd(matrix.Vec[0][0], vecNTT.value[0], res.value[0]) // res += c0_Q * plaintext
		ringQ.MulCoeffsMontgomeryAndAdd(matrix.Vec[0][0], vecNTT.value[1], res.value[1]) // res += c1_Q * plaintext
	}

	ringQ.InvNTT(res.value[0], res.value[0])
	ringQ.InvNTT(res.value[1], res.value[1])
}
...
func (eval *fvEvaluator) MultiplyByDiabMatrixBSGS(vecNTT, res *Ciphertext, matrix *PtDiagMatrixT, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
	...
	ringQ.MulScalarBigint(vecNTT.value[0], ringP.ModulusBigint, tmpQ0) // P*c0
	...
	for _, i := range rotations {
		if i != 0 {
			...
			ring.PermuteNTTWithIndexLvl(levelQ, tmpQ0, index, tmpQ1) // phi(P*c0)
			ringQ.Add(vecRotQ[i][0], tmpQ1, vecRotQ[i][0])           // phi(d0_Q) += phi(P*c0)
		}
	}
		...
		if j != 0 {
			...
			for _, i := range index[j] {
				...
				if i == 0 {
					...
					if cnt1 == 0 {
						ringQ.MulCoeffsMontgomeryConstant(plaintextQ, vecRotQ[i][0], tmpQ0)  // phi(P*c0 + d0_Q) * plaintext
						ringQ.MulCoeffsMontgomeryConstant(plaintextQ, vecRotQ[i][1], tmpQ1)  // phi(d1_Q) * plaintext
						...
						ringQ.MulCoeffsMontgomeryConstantAndAddNoMod(plaintextQ, vecRotQ[i][0], tmpQ0) // phi(d0_Q) * plaintext
						ringQ.MulCoeffsMontgomeryConstantAndAddNoMod(plaintextQ, vecRotQ[i][1], tmpQ1) // phi(d1_Q) * plaintext
					...
					if cnt1%(QiOverF>>1) == (QiOverF>>1)-1 {
						ringQ.Reduce(tmpQ0, tmpQ0)
						ringQ.Reduce(tmpQ1, tmpQ1)
					}
			...
			if state {
				...
				if len(index[j]) == 1 {
					ringQ.MulCoeffsMontgomery(matrix.Vec[N1*j][0], vecNTT.value[0], tmpQ0) // c0 * plaintext + sum(phi(d0) * plaintext)/P + phi(c0) * plaintext mod Q
					ringQ.MulCoeffsMontgomery(matrix.Vec[N1*j][0], vecNTT.value[1], tmpQ1) // c1 * plaintext + sum(phi(d1) * plaintext)/P + phi(c1) * plaintext mod Q
				} else {
					ringQ.MulCoeffsMontgomeryAndAdd(matrix.Vec[N1*j][0], vecNTT.value[0], tmpQ0) // c0 * plaintext + sum(phi(d0) * plaintext)/P + phi(c0) * plaintext mod Q
					ringQ.MulCoeffsMontgomeryAndAdd(matrix.Vec[N1*j][0], vecNTT.value[1], tmpQ1) // c1 * plaintext + sum(phi(d1) * plaintext)/P + phi(c1) * plaintext mod Q
				}
			...
			ringQ.Add(res.value[0], tmpQ1, res.value[0])             // res += phi(tmpRes)
			...
			if cnt0%QiOverF == QiOverF-1 {
				ringQ.Reduce(tmpQ2, tmpQ2)
				ringQ.Reduce(tmpQ3, tmpQ3)
			}
	...
	for _, i := range index[0] {
		...
		if i == 0 {
			...
			N1Rot++
			// keyswitch(c1_Q) = (d0_QP, d1_QP)
			ringQ.MulCoeffsMontgomeryConstantAndAddNoMod(plaintextQ, vecRotQ[i][0], tmpQ2) // phi(P*c0 + d0_Q) * plaintext
			ringQ.MulCoeffsMontgomeryConstantAndAddNoMod(plaintextQ, vecRotQ[i][1], tmpQ3) // phi(d1_Q) * plaintext
	...
	ringQ.Add(res.value[0], tmpQ2, res.value[0]) // res += sum(phi(c0 * P + d0_QP))/P
	ringQ.Add(res.value[1], tmpQ3, res.value[1]) // res += sum(phi(d1_QP))/P
