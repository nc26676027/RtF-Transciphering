...
// FVEvaluator is an interface implementing the public methodes of the eval.
type FVEvaluator interface {
...
func newEvaluatorPrecomp(params *Parameters) *fvEvaluatorBase {
	var err error
	ev := new(fvEvaluatorBase)
...
type fvEvaluatorBuffers struct {
	poolQ    [][]*ring.Poly
	poolQmul [][]*ring.Poly
...
func newEvaluatorBuffer(eval *fvEvaluatorBase) *fvEvaluatorBuffers {
	evb := new(fvEvaluatorBuffers)
...
// NewFVEvaluator creates a new Evaluator, that can be used to do homomorphic
// operations on ciphertexts and/or plaintexts. It stores a small pool of polynomials
// and ciphertexts that will be used for intermediate values.
func NewFVEvaluator(params *Parameters, evaluationKey EvaluationKey) FVEvaluator {
	ev := new(fvEvaluator)
	ev.fvEvaluatorBase = newEvaluatorPrecomp(params)
	ev.fvEvaluatorBuffers = newEvaluatorBuffer(ev.fvEvaluatorBase)
...
func NewFVEvaluators(params *Parameters, evaluationKey EvaluationKey, n int) []FVEvaluator {
	if n <= 0 {
		return []FVEvaluator{}
	}
	evas := make([]FVEvaluator, n, n)
	for i := range evas {
		if i == 0 {
			evas[0] = NewFVEvaluator(params, evaluationKey)
		} else {
...
func (eval *fvEvaluator) ShallowCopy() FVEvaluator {
	return &fvEvaluator{
		fvEvaluatorBase:    eval.fvEvaluatorBase,
...
func (eval *fvEvaluator) WithKey(evaluationKey EvaluationKey) FVEvaluator {
	return &fvEvaluator{
		fvEvaluatorBase:    eval.fvEvaluatorBase,
		fvEvaluatorBuffers: eval.fvEvaluatorBuffers,
...
func (eval *fvEvaluator) Add(op0, op1 Operand, ctOut *Ciphertext) {
...
func (eval *fvEvaluator) AddNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, utils.MaxInt(op0.Degree(), op1.Degree()))
	eval.Add(op0, op1, ctOut)
	return
}

// AddNoMod adds op0 to op1 without modular reduction, and returns the result in cOut.
func (eval *fvEvaluator) AddNoMod(op0, op1 Operand, ctOut *Ciphertext) {
	el0, el1, elOut := eval.getElemAndCheckBinary(op0, op1, ctOut, utils.MaxInt(op0.Degree(), op1.Degree()), true)
...
func (eval *fvEvaluator) AddNoModNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, utils.MaxInt(op0.Degree(), op1.Degree()))
...
func (eval *fvEvaluator) Sub(op0, op1 Operand, ctOut *Ciphertext) {
	el0, el1, elOut := eval.getElemAndCheckBinary(op0, op1, ctOut, utils.MaxInt(op0.Degree(), op1.Degree()), true)
...
func (eval *fvEvaluator) SubNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, utils.MaxInt(op0.Degree(), op1.Degree()))
...
func (eval *fvEvaluator) SubNoMod(op0, op1 Operand, ctOut *Ciphertext) {
	el0, el1, elOut := eval.getElemAndCheckBinary(op0, op1, ctOut, utils.MaxInt(op0.Degree(), op1.Degree()), true)
...
func (eval *fvEvaluator) SubNoModNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, utils.MaxInt(op0.Degree(), op1.Degree()))
...
func (eval *fvEvaluator) Neg(op Operand, ctOut *Ciphertext) {
...
func (eval *fvEvaluator) NegNew(op Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, op.Degree())
	eval.Neg(op, ctOut)
	return ctOut
}

// Reduce applies a modular reduction to op and returns the result in ctOut.
func (eval *fvEvaluator) Reduce(op Operand, ctOut *Ciphertext) {
	el0, elOut := eval.getElemAndCheckUnary(op, ctOut, op.Degree())
...
func (eval *fvEvaluator) ReduceNew(op Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, op.Degree())
...
func (eval *fvEvaluator) MulScalar(op Operand, scalar uint64, ctOut *Ciphertext) {
	el0, elOut := eval.getElemAndCheckUnary(op, ctOut, op.Degree())
...
func (eval *fvEvaluator) MulScalarNew(op Operand, scalar uint64) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, op.Degree())
...
func (eval *fvEvaluator) tensorAndRescale(ct0, ct1, ctOut *Element) {
...
func (eval *fvEvaluator) modUpAndNTT(ct *Element, cQ, cQMul []*ring.Poly) {
...
func (eval *fvEvaluator) tensoreLowDeg(ct0, ct1 *Element) {
...
func (eval *fvEvaluator) tensortLargeDeg(ct0, ct1 *Element) {
...
func (eval *fvEvaluator) quantize(ctOut *Element) {
...
func (eval *fvEvaluator) Mul(op0 *Ciphertext, op1 Operand, ctOut *Ciphertext) {
...
func (eval *fvEvaluator) mulPlaintextMul(ct0 *Ciphertext, ptRt *PlaintextMul, ctOut *Ciphertext) {
	for i := range ct0.value {
...
func (eval *fvEvaluator) mulPlaintextRingT(ct0 *Ciphertext, ptRt *PlaintextRingT, ctOut *Ciphertext) {
	ringQ := eval.ringQ
...
func (eval *fvEvaluator) MulNew(op0 *Ciphertext, op1 Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, op0.Degree()+op1.Degree())
...
func (eval *fvEvaluator) relinearize(ct0 *Ciphertext, ctOut *Ciphertext) {
...
// - it must be of degree high enough to relinearize the input ciphertext to degree 1 (e.g., a ciphertext
// of degree 3 will require that the evaluation key stores the keys for both degree 3 and degree 2 ciphertexts).
func (eval *fvEvaluator) RelinearizeNew(ct0 *Ciphertext) (ctOut *Ciphertext) {
...
func (eval *fvEvaluator) SwitchKeys(ct0 *Ciphertext, switchKey *SwitchingKey, ctOut *Ciphertext) {
...
// it must encrypt the target key under the public key under which ct0 is currently encrypted.
func (eval *fvEvaluator) SwitchKeysNew(ct0 *Ciphertext, switchkey *SwitchingKey) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, 1)
...
func (eval *fvEvaluator) RotateColumns(ct0 *Ciphertext, k int, ctOut *Ciphertext) {
...
func (eval *fvEvaluator) RotateColumnsNew(ct0 *Ciphertext, k int) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, 1)
...
func (eval *fvEvaluator) RotateRows(ct0 *Ciphertext, ctOut *Ciphertext) {
...
func (eval *fvEvaluator) RotateRowsNew(ct0 *Ciphertext) (ctOut *Ciphertext) {
	ctOut = NewCiphertextFV(eval.params, 1)
...
func (eval *fvEvaluator) InnerSum(ct0 *Ciphertext, ctOut *Ciphertext) {

	if ct0.Degree() != 1 || ctOut.Degree() != 1 {
		panic("cannot InnerSum: input and output must be of degree 1")
	}

	cTmp := NewCiphertextFV(eval.params, 1)
...
func (eval *fvEvaluator) permute(ct0 *Ciphertext, generator uint64, switchKey *rlwe.SwitchingKey, ctOut *Ciphertext) {
...
func (eval *fvEvaluator) switchKeysInPlace(cx *ring.Poly, evakey *rlwe.SwitchingKey, pool2Q, pool3Q *ring.Poly) {
...
func (eval *fvEvaluator) getRingQElem(op Operand) *Element {
	switch o := op.(type) {
	case *Ciphertext, *Plaintext:
		return o.El()
	case *PlaintextRingT:
		fvScaleUp(eval.ringQ, eval.deltaMont, o.value, eval.tmpPt.value)
...
func (eval *fvEvaluator) getElemAndCheckBinary(op0, op1, opOut Operand, opOutMinDegree int, ensureRingQ bool) (el0, el1, elOut *Element) {
...
func (eval *fvEvaluator) getElemAndCheckUnary(op0, opOut Operand, opOutMinDegree int) (el0, elOut *Element) {
...
func (eval *fvEvaluator) evaluateInPlaceBinary(el0, el1, elOut *Element, evaluate func(*ring.Poly, *ring.Poly, *ring.Poly)) {
...
func (eval *fvEvaluator) decomposeAndSplitNTT(level, beta int, c2NTT, c2InvNTT, c2QiQ, c2QiP *ring.Poly) {
